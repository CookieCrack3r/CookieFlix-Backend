...
class VideoItemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not user.is_authenticated:
            return Video.objects.none()  # Gebe ein leeres QuerySet zurück, wenn der Benutzer nicht eingeloggt ist
        return Video.objects.filter(models.Q(users=user) | models.Q(is_public=True))

    
    def perform_create(self, serializer):
        video = serializer.save()
        video.users.add(self.request.user)
...
def grouped_videos(request):
    ...
    if not request.user.is_authenticated:
        return JsonResponse({'error': 'Benutzer ist nicht authentifiziert'}, status=403)

    # Nur wenn der Benutzer authentifiziert ist, filtere die Videos
    categories = Category.objects.filter(
        models.Q(videos__users=user) | models.Q(videos__is_public=True)
    ).distinct()
    ...
    for category in categories:
        videos = Video.objects.filter(
            models.Q(category=category) & (models.Q(users=user) | models.Q(is_public=True))
        )
